...
BOOL CALLBACK EnumWndProc(HWND hWnd, LPARAM lParam) {
  ...
  if (strcmp(buff, (reinterpret_cast<Pack*>(lParam))->WindowName) == 0) {
    (reinterpret_cast<Pack*>(lParam))->hWnd = hWnd;
  }
  return true;
}
...
Uint32 getpixel(SDL_Surface *surface, int x, int y) {
  ...
  Uint8 *p = reinterpret_cast<Uint8*>(surface->pixels) +
                                      y * surface->pitch +
                                      x * bpp;
...
bool jdgMinoAru(int x, int y) {
  const int bx = 101;
  const int by = 136;
  unsigned c = getpixel(suf, bx+MINO_WIDTH*x+MINO_WIDTH/2,
                        by+MINO_HEIGHT*y+MINO_HEIGHT/2);
  /*
  //printf("0x%08x\n",c);
  if( c != 0x1a1a1aff ) {
    //printf("o");
  ...
  case THINK:
    {
      ...
      if (!match && false) {
        ...
        if ( file < 50 ) {
          char filename[256];
          snprintf(filename, sizeof(filename), "%d.bmp", file++);
      ...
      printf("x:%d y:%d r:%d h:%d next:%d\n", dc.x, dc.y, dc.r, dc.h,
             nekumino[0]);
      Simulator::putMino(&f, MinoType(nekumino[0]), dc);
      puts("risou");
...
void sim(HWND hwnd) {
  ...
  while (!quit) {
    ...
    GetDIBits(hdc, hBMP, 0, dwHeight, lpPixel, (LPBITMAPINFO)lpInfo,
              DIB_RGB_COLORS);

    ReleaseDC(hwnd, hdc);
    DeleteObject(hBMP);
    DeleteObject(hdcMem);
    suf = load_image(reinterpret_cast<char*>(lpBuf), dwFSize);
    GlobalFree(lpBuf);
...
int main(int argc, char* argv[]) {
  ...
  c.hWnd = NULL;
  snprintf(c.WindowName, sizeof(c.WindowName), "%s", "TetrisOnline");
  ...
  if (c.hWnd != NULL) {
    ...
    TTF_Init();
#ifdef USE_GUI
    window = SDL_CreateWindow("teto", SDL_WINDOWPOS_UNDEFINED,
